...
이론에 대한 확신에 따라 사용자 검토 지점의 밀도를 조절합니다. TDD 3법칙·한 번에 테스트 하나·테스트 삭제 금지는 모든 기어에서 동일하고, **커밋 단위는 기어를 따라 바뀝니다**.

| 기어 | 검토 지점 | 커밋 단위 | 대응 상황 |
|------|-----------|-----------|-----------|
...
완료 보고 시 하드닝 제안(선택)을 함께 제시한다 — crap4java·dry4java·mutate4java로
실행하는 명령을 제안만 하고(설치는 `msbaek-tdd/references/hardening-tools.md`),
