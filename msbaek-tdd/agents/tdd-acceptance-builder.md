...
tools: Edit, MultiEdit, Write, Read, Grep, Glob, Bash(gradle test:*), Bash(./gradlew:*), Bash(mvn test:*), Bash(./mvnw:*), Bash(git status:*), Bash(git diff:*), Bash(git add:*), Bash(git commit:*)
model: sonnet
---
...
- **Scenario Outline은 Examples 블록 단위로 분리한다** — 여러 규칙이 한 표에 있으면 한 사이클에
