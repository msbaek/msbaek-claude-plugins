...
tools: Edit, MultiEdit, Write, Read, Bash(gradle test:*), Bash(./gradlew:*), Bash(mvn test:*), Bash(./mvnw:*), Bash(git status:*), Bash(git diff:*), Bash(git add:*), Bash(git commit:*)
model: sonnet
---
...
계산되어야 한다. 실제 계산된 값을 복사해 기대값에 붙여넣기를 금지한다. 예: `1+3`이면
...
**사각지대**다 — 필드를 골라 단정하면 고르지 않은 필드는 검증에서 누락되고, 누락되었다는
사실이 테스트에 드러나지 않는다.

> 실측: HTTP 응답 DTO의 `unitPrice`를 `long`→`double`로 바꾸자 승인 검증은 실패했지만
...
분기에 쓰는 본문이 검증에서 누락된다. 프레임워크 기본 에러 응답이 그대로 반환되고 있어도,
버전을 올려 형식이 바뀌었을 때 아무 테스트도 실패하지 않는다.

**두 종류의 승인을 섞지 않는다 — 탐지 대상이 다르다**:
...
직렬화 형식이 바뀌면 raw 구획이 실패하고, 사람이 읽는 형태가 바뀌면 printer 구획이
실패한다. 실패 시 diff가 어느 층위에서 실패했는지도 알려준다 — raw만 실패하면 표현만
바뀐 것, 둘 다 실패하면 값 자체가 바뀐 것.

> 실측: `refundAmount`를 `long`→`double`로 바꾸자 raw 구획은 실패했지만(`20000`→`20000.0`)
> printer 구획은 `20,000원` 그대로였다(금액 포맷이 타입 변화를 흡수). 비공허성 확인:
> `clock`에 +1시간 주입 시 raw·printer 둘 다 실패했다.

조건: **사용자에게 보여지는 출력**(주문·결제·정산 결과 등)이면 응답이 짧은 한 기본으로
병기한다 — Walking Skeleton도 포함이다(raw는 그대로 두고 printer 구획을 더한다).
사용자에게 노출되지 않는 내부 API는 raw만으로 충분하다. 상한은 그대로 — 응답이 커지면
...
이름). 원천 문서에 타임존 정책이 없어도 printer는 KST로 출력한다 — API 응답 계약이 UTC인
