...
- 예제: 값의 정본인 검산표 — 대표 입력의 단계별 계산 결과를 표로, 각 행에 적용되는 규칙 번호
...
2. **경계 조건 5종 스캔** — 수치·크기·상태·시간·집계 경계를 검토하고, 해당하는 것만
...
5. **핵심 예시만** — 망라적 edge 나열 금지. Gherkin에 없는 세밀 분기는 목록으로
   만들지 않는다 — 구현 중 발견 시 `../references/anchor-update.md`의 "테스트 목록 갱신"으로
   예제 행(기존 규칙) 또는 규칙(새 동작)에 추가된다
