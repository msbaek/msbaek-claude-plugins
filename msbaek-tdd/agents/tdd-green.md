...
1. **테스트가 너무 큰 도약인지 먼저 판단** — 그렇다면 simpler test로 후퇴(Red로 복귀,
   ...
   해결되지 않을 수 있다. **후퇴 처리 절차**: 테스트를 삭제하지 않고
...
- **구현 복잡도**: 단순(1~2줄) → Obvious / 중간(3~5줄) → 가능하면 Obvious, 구현이 진행되지 않으면 Fake it /
  복잡(6줄+) → 무조건 Fake it
...
- **테스트 통과 후 기존 테스트가 실패함** → 커밋하지 않고 원인 확인. 최소 구현 범위를 넘었을
  가능성
...
- Getting Stuck 후퇴는 오케스트레이터에 명시적으로 보고한다 — 보고하지 않으면 다음 사이클이
  잘못된 목록 순서로 진행된다
