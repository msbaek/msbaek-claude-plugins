...
#### Scenario Outline — Examples 블록 단위로 쪼개 한 걸음씩 해제
...
한 사이클에서 동시에 green으로 만들어야 한다 — TDD의 한 걸음으로는 너무 크다.
행을 블록으로 쪼개면 사이클마다 한 규칙씩 통과시킬 수 있다:
...
1. 새 시나리오를 failing test로 추가할 때 **쪼갠다**(각 블록에 `@pending`)
2. Green 단계마다 자기 블록의 `@pending`을 같은 커밋에서 **뗀다**
3. 그 Outline이 **전부 green이 되면 한 블록으로 다시 합친다** — 이때 표 헤더가
   한 번만 남는 것이 정상 상태다. 합치는 주체와 시점은 **마지막 `@pending`을 뗀 그
   Green과 같은 커밋**이다 (별도 정리 단계를 두면 아무도 하지 않아 누락된다)
4. 이후 새 시나리오를 추가할 때 다시 1번으로

3번을 빼먹으면 헤더가 반복된 채 블록이 계속 누적되어, 표 하나로 읽히던 Examples가
읽기 어려워진다. 전부 green인데 블록이 여럿이면 **합치지 않은 것이지 의도된 구조가
아니다.**

### 기존 JUnit 인수 테스트 이관 (기존 프로젝트)

이관은 반복적·기계적 작업이라 사람이 손으로 하기엔 지루하다 — 이 스킬을 호출하면
