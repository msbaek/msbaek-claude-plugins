...
4. **커밋 2 (`feat:`)** — `git add` 후 커밋. subject `feat(<범위>): <feature 이름>`.
   feature의 모든 test 체크박스를 `- [x]`로 갱신해 **같은 커밋에** 담는다.
5. **Blue 전체** — `tdd-blue`에 이 feature 범위 전체(커밋 2 이후 변경 파일 목록)로
   위임한다. Local Tidying Process. **커밋 보류**. 적용 후에도 스위트 green 유지를
   확인한다.
6. **커밋 3 (`refactor:`)** — 변경이 있으면 `git add` 후 커밋. 변경이 없으면(이미
   깔끔하면) 생략한다 — 억지로 만들지 않는다.

각 phase 내부(1, 3, 5)에서 test마다 에이전트가 반환한 변경 요약(무엇을·왜 그렇게
