...
2. **적대적 리뷰 1회(기어 무관, 협상 대상 아님)** — "그리고 어느 기어로 진행하든
   전체 완료 시 적대적 리뷰를 1회 실행합니다." 아래 "적대적 리뷰" 섹션.
...
4. **기어 결정**: `--gear` 파라미터 > 템플릿 문서의 기어 기록 > 기본 low 순으로 결정.
   폭발 반경 점검("기어(Gears)" 섹션)을 수행하고, high-stakes 영역이면 두 처방을 함께
...
#### Step 3: 완료 처리
- 체크박스 확인 (`- [x]`) — **체크는 Red 커밋에서 이미 찍힌다**(`tdd-red` 5·6단계).
  체크박스의 뜻은 "이 테스트를 Red로 추가했다"이지 "이 테스트가 green이다"가 아니다.
  아직 `- [ ]`이면 Red 커밋에서 누락된 것이므로 여기서 채운다
- **완료 판정은 체크박스만으로 하지 않는다** — 마지막 테스트가 Red만 된 상태에서도
  "모두 `[x]`"가 성립한다. 전체 스위트 green을 함께 확인한다
- 작업 내역을 마크다운 파일에 반영
...
- 다음 테스트 안내 또는 전체 완료
- **전체 완료 시 high 기어였거나 폭발 반경 high-stakes였다면**: Web App의 경우 추가
  단계(인수 테스트 완료 확인· JPA Repository 완성·DSL 개선)까지 완료한 뒤 적대적 리뷰를
  실행한다 — 리뷰 diff가 전체 구현을 포함해야 하기 때문. 최종 사용자 검토 전에 적대적
  리뷰를 실행 (아래 "적대적 리뷰" 섹션). green 스위트 + 적대적 리뷰 통과가 이 경우의
  Definition of Done. high 기어라면 최종 검토 보고에 표본 정독 대상으로 대표
  슬라이스 하나(가장 복잡했거나 후퇴가 있었던 것)를 추천해 함께 제시
...
### 적대적 리뷰 (high 기어 또는 폭발 반경 high-stakes 영역)
