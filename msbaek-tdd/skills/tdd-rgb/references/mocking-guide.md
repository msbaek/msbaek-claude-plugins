...
> `tdd-rgb` 스킬의 참조 문서. Blue 단계에서 테스트 더블 도입·제거를 판단할 때,
> 또는 "이걸 mocking해도 되나"라는 질문이 생겼을 때 읽는다.
> SKILL.md에는 판단에 바로 쓰는 규칙만 남기고, 근거·인용·안티패턴 목록은 여기에 둔다.
...
- **취약한 Mock들**: 실제 외부 시스템의 동작이 변경되면 Mock들이 테스트를 실패시킬 수 있음
...
- 코드 구현이 조금만 변경되어도 테스트가 실패할 수 있음
...
- Mock Roles not Objects > _Role-based Design_
  ...
  - 이걸 지켜야 리팩터링을 해도 mock test가 실패하지 않음
...
- solitary test(mocking)는 쉽게 실패함
...
- 테스트의 실패하기 쉬움을 최소화하기 위해 모킹보다 실제 객체나 Fake 객체 사용을 고려하라
