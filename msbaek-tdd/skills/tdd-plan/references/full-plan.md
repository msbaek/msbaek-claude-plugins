...
대형·다팀 작업에서 상세 명세가 정당화될 때만 쓴다. 기본 경량 플로우는
`../SKILL.md`가 정본이다.
...
> 자연히 식별되지만, **같은 키가 여러 항목으로 분리된 경우**는 식별되지 않는다. 검증은
> 항목별로 하고 반영은 누적으로 하면 그 사이에서 차이가 발생한다:
>
> ```
...
- **세밀한 분기** — null·empty·미매핑 입력 등 커버리지용 (Gherkin에 넣으면 가독성 저하)
...
리팩터링 시 실패하기 쉬운 비용을 감수하고 커버리지·엣지케이스를 얻는 의도적
...
unit test(세밀 분기)가 물리적으로 한 목록에 혼재하지만, 개념적 구분은 유지된다.
...
복잡도가 **규칙**에 있으면 0층(1a)의 분량이 늘고 Use Case는 줄어든다 — 계산 중심
도메인은 생략이 기본이다. 복잡도가 **흐름·상태 전이**에 있으면 Use Case의 확장절이
중심이 된다 — 주로 `web-app` 유형에서 해당한다.
