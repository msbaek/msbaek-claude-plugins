...
누적되어 있다 — LAZY 접근이 예외를 발생시키는 지점마다 EAGER로 바꾸는 식으로 확산되기 때문이다.
...
항목이 아예 없으면 Spring Boot 기본값 `true`가 적용된다. 설정 부재를 off로 가정하지 않는다 —
**적어야 꺼진다.** 이 항목이 없는 채로 개발하면 켜진 상태를 모르고 그 위에서 설계하게
되고, 나중에 되돌리는 비용이 3단계로 누적된다: LAZY 접근이 우연히 성공 → 예외가
발생하는 지점을 EAGER로 우회 → 경계를 확정하면서 EAGER를 도로 LAZY로 되돌리기.

OSIV가 켜져 있으면 영속성 컨텍스트가 뷰 렌더링까지 열려 있어, 트랜잭션 밖에서도 LAZY
접근이 우연히 성공한다. 경계가 어디인지 코드로 드러나지 않고, DB 커넥션이 요청 처리
내내 점유된다. 끄면 경계를 명시해야 한다:
...
- **연관관계는 LAZY를 유지한다** — "OSIV 밖에서 예외가 발생하니까 EAGER로"는 우회다. EAGER는
  전역 결정이라 목록 조회가 생기는 순간 N+1이 되고, 그때는 되돌리기 어렵다. 필요한
  지점에서 fetch join·`@EntityGraph`로 **명시적으로** 로딩한다. 조회 메서드가 애그리게이트를
  돌려준다면 그 구성물을 함께 로딩하는 것이 기본이다(아래 계약 테스트 항목 참조)
- **저장은 명시적 `save()` 호출을 관례로 한다** — JPA는 managed entity의 변경을 커밋
  ...
  "`save()` 없는 변경이 누출되지 않는다"가 **항상 통과한다**(위험 경로를 한 번도
  실행하지 않는 공허한 검증)

**적용 순서 — 쓰기 경로에서는 가드가 경계보다 먼저 도입되어야 한다.**

Controller에 트랜잭션 경계가 없는 동안에는 리포지토리 메서드 단위로 커밋되어 조회
결과가 곧바로 detached이므로, `save()` 없는 변경은 애초에 누출될 수 없다 —
**위험이 없으니 가드도 없는 상태가 정상으로 보인다.** 그러다 경계를 Controller로 이동하는
순간 엔티티가 요청 내내 managed로 남고 커밋 시 dirty checking이 flush하므로, **그 변경
하나가 누출 경로를 만든다.** 그 시점에 가드가 없으면 새 위험을 탐지할 장치 없이 위험만 켜진다.
...
1. **로직 없는 pass-through** - 저장하고 그대로 돌려준다. 계산이 필요한 시나리오는
   ...
   - **단, 반환 타입은 엔티티가 아니라 DTO다** (위 skeleton 테스트의
     ...
     *엔티티를 그대로 노출한다*는 뜻이 아니다. OSIV를 끈 상태에서 엔티티를 반환하면
     ...
     트랜잭션 밖이고, LAZY 연관에 접근하는 순간 `LazyInitializationException`(HTTP 500)이다.
     **이 실패는 테스트에서 안 보인다** — skeleton 테스트는 클래스 레벨
     `@Transactional`이라 직렬화까지 테스트 트랜잭션 안에서 끝난다. 테스트는 초록색,
     실서버는 500. DTO 매핑을 트랜잭션 경계 안에서 끝내는 것이 이 경로를 차단한다
...
4. **예외 처리는 처음부터 분리** - `@RestControllerAdvice` 전역 핸들러에 둔다. 컨트롤러
   안에 `@ExceptionHandler`를 두지 않는다 — 컨트롤러가 늘어나면 같은 처리가 분산된다.
...
**어노테이션 선언 위치가 곧 동작을 결정한다 — 하위 클래스에 붙이면 무시된다.**
...
"조용한 실패"). 같은 이유로
`@DataJpaTest`가 주는 트랜잭션도 상속 메서드에는 적용 여부가 달라지므로, **의도를
...
적용되지 않는다**. 따라서 부모의 `NOT_SUPPORTED`를 제거해도 계약 테스트는 이미 트랜잭션
밖이라 아무것도 red가 되지 않는다 — "제거하면 공허해진다"는 예측은 틀렸고, 그
선언은 오늘 시점엔 **방어적 명시**다(부모에 순수 `@Transactional`을 넣으면 적용되므로
위치 규칙 자체는 맞다). 결론 두 가지:

- **왕복 단언은 트랜잭션 안/밖 모두 통과한다** — 행위 단언만으로는 이 불변조건을
  고정할 수 없다. `TestTransaction.isActive() == false`를 직접 단언하는 **guard test**를
  계약에 둔다(위 예시 `runsOutsideTransaction`). 부모에 `@Transactional`을 추가하면
  guard만 red가 되는 것으로 민감함을 확인했다.
- 같은 계약이 in-memory 어댑터의 숨은 버그(Map 참조를 그대로 반환 → 호출자 mutate 시
  저장소 오염)도 검출했다 — 단, identity/snapshot을 **명시적으로 단언한 뒤에야**.
  계약이 검출해야 할 차이(트랜잭션 경계·identity·null 처리)는 나열하고 항목마다 실패
  주입으로 red를 확인한다.

트랜잭션을 켠 채로 왕복을 검증하려 하면 우회가 다층으로 누적된다 — `flush()` + `clear()`로
...
**트랜잭션 밖 실행의 대가 세 가지** (롤백이 없어지면서 발생한다):
...
- **LAZY 연관을 그대로 두면 검증할 수 없다** — 세션이 닫혀 있어 접근 시
  `LazyInitializationException`이다. 여기서 연관을 검증 대상에서 **제외하지 않는다** —
  제외하면 그 부분 계약이 어디에서도 검증되지 않는다. 대신 **리포지토리 포트의 계약을
  "완전한 애그리게이트를 돌려준다"로 정하고**, 조회 메서드에 `@EntityGraph`·fetch join을
  걸어 조회 시점에 명시적으로 로딩한다. 매핑은 LAZY로 두고 조회 지점에서 명시하는 것이라
