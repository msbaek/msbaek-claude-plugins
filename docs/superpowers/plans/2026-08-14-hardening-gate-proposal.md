...
- **도구 우선순위**: CRAP·DRY는 빠른 2종으로 제안 1순위(변경 파일 한정 명령 제시), mutation은 실행 비용이 크므로 비용 경고와 함께 별도 표기. (사용자 결정 2026-08-14)
  > **2026-08-20 v1.40.1에서 대체됨** — 이 "비용순" 묶음이 mutation을 구조 정리보다
  > 먼저 돌게 만든다는 것이 드러나, 파이프라인순(CRAP·DRY → 구조 정리 → mutation)으로
  > 재구성됐다. 아래 "순서 정정" 절 참조. 이 항목과 Task 1 §2의 블록 형식은 당시
  > 기록으로 남겨둔 것이며 현재 정본이 아니다.
...
## 사후 발견 — 연결 누락과 후속 수정 (2026-08-20, v1.40.0)

이 plan은 Task 2에서 **tdd-feature 한 경로에만** 게이트를 연결했다. 정본을
`skills/tdd-rgb/references/`에 둔 것은 기존 관례("tdd-feature가 소비하는 정본들이
이미 이곳에 있음")를 따른 것이었지만, 그 배치가 **tdd-rgb도 연결됐다는 착각**을 낳았다.
실제로는 `tdd-rgb/SKILL.md`에 `hardening-gate` 참조가 한 줄도 없어(grep 0건),
`/tdd-rgb`로 완주하면 기어와 무관하게 하드닝 제안이 나올 수 없는 상태였다.

**발견 경위**: 검증 프로젝트(`tdd-plugin-verifier` 세션, `tdd-rgb-high` 브랜치)에서
Cart 결제 금액 계산 use case를 high 기어로 완주했는데 CRAP·DRY·mutation 제안이 한 번도
제시되지 않았다. 조건 미충족(§1 Java·Maven·변경파일) 생략이 아니라 — 그 프로젝트는
Maven이고 `src/main/java` 변경도 있었다 — 게이트를 호출하는 코드 경로 자체가 없었던 것.
"적대적 리뷰는 수행됨 / 하드닝 3종은 미수행"이라는 그 브랜치의 상태는 검증 절차의
선택이 아니라 이 누락의 결과였다(검증 repo `Cart.md` §6에 커밋 `c1dcbf9`로 기록).

**수정** (커밋 `87957c4`, v1.39.1 → v1.40.0):

1. `tdd-rgb/SKILL.md` Step 3(완료 처리)에 "하드닝 제안 (실행 아님, 모든 기어)" 추가 —
   최종 검토 보고 마지막에 제안 블록, 자동 실행은 여전히 금지
2. "조건부 절차 — references" 표에 hardening-gate.md 행 추가
3. FAILURE 체크리스트에 "제안 블록을 붙였는가 / 자동 실행하지 않았는가" 추가
4. `hardening-gate.md` 머리말을 tdd-feature 전용 → 양쪽 스킬 공용으로 갱신하고,
   diff 기준점을 호출 스킬별로 일반화(tdd-feature는 Phase B 시작 해시, tdd-rgb는
   적대적 리뷰 기준점 — 없으면 이 작업 첫 커밋의 부모)

**삽입 위치 결정과 버린 대안**:

- 채택: Step 3 완료 처리의 최종 검토 보고 마지막 (모든 기어)
- 버림 ①: `adversarial-review.md`에 통합 — 양쪽 스킬이 한 곳만 참조하게 되어 중복은
  사라지지만, 적대적 리뷰가 돌지 않는 경로(low·mid 기어 + 폭발 반경 비위험)에서는
  제안이 영영 나오지 않는다
- 버림 ②: `/system-wide-refactoring` 완료 후 — 그 스킬은 선택 호출이라 부르지 않으면
  제안 기회 자체가 없고, CRAP·DRY는 리팩토링 **대상을 알려주는 입력**이므로 그 이전에
  도는 것이 맞다. 리팩토링 후에는 검증용 재실행을 쓴다

**교훈**: reference 파일을 스킬 디렉터리에 두는 것은 소속을 표현할 뿐 실행 경로를
만들지 않는다. 완료 지점 게이트를 추가할 때는 `grep -rn <reference명> skills/`로
**모든 오케스트레이터**가 참조하는지 확인하고, 한 경로에만 있으면 의도(경로별 차등)인지
누락인지 명시적으로 판단한다.

---

## 순서 정정 — 비용순에서 파이프라인순으로 (2026-08-20, v1.40.1)

위 연결 수정 직후, 이 게이트의 원천 아이디어인 Uncle Bob의 실제 파이프라인과 대조한
결과 **블록 내부 순서**가 어긋나 있음이 드러났다. 삽입 위치(완료 보고)는 맞았다.

**원천과의 대조** (vault `001-INBOX/LIVE Uncle Bob on Software Fundamentals in the
Age of AI.md` [18:01-24:03] "멀티 에이전트 파이프라인"):

| Uncle Bob 5단계 | 이 플러그인의 대응 | 판정 |
|---|---|---|
| Specifier (Gherkin + QA 절차) | `/tdd-plan`·`/cucumber-acceptance` | — |
| Coder (유닛 테스트 + 프로덕션 코드) | RGB 완주 | ✓ |
| **Cleaner** (CRAP 분석 + 코드 리뷰 → 정리) | CRAP·DRY 제안 → `/system-wide-refactoring` | ✓ |
| **Hardener** (뮤테이션 무자비하게) | mutation을 CRAP·DRY와 **같은 블록에** 나란히 제안 | ✗ |
| QA agent (QA 문서 → 실행 스크립트) | 대응 없음 | — |

이 plan의 §2 제안 블록은 셋을 **비용순**("빠른 2종 / 느린 1종")으로 묶었다. 위에서부터
실행하면 mutation이 구조 정리 **전에** 돌아간다 — Uncle Bob의 순서에서는 Hardener가
Cleaner **다음**이다.

**왜 순서가 실제로 문제인가**: tidying은 동작을 보존하므로 테스트는 그대로지만
**뮤테이션 지점은 바뀐다**. 메서드를 추출하면 새 뮤턴트 자리가 생겨서, 정리 전에 얻은
"전 뮤턴트를 죽였다"는 결론이 정리 후에는 성립하지 않는다. 파일당 수 분이 드는
비용을 곧 재구조화될 코드에 쓰는 것도 그대로 버려진다.

**수정** (커밋 `eb55fff`, v1.40.0 → v1.40.1): 제안 지점은 완료 보고 한 곳으로 유지하고
블록 안에 순서를 담았다 — ①정리할 곳 찾기(CRAP·DRY) → ②구조 정리
(`/system-wide-refactoring`) → ③테스트 강화(mutation). ②를 건너뛸 경우 ③을 바로
실행해도 된다는 탈출구를 명시했고, 근거와 출처를 블록 밖 주석으로 남겼다(다음에
비용순으로 되돌리려는 사람이 이유를 먼저 읽도록). Gradle 분기도 새 번호 체계에 맞춰
정정했다(①의 CRAP과 ③ 전체 제외).

**여기서도 버린 대안**: 제안 지점을 둘로 쪼개 mutation을 `/system-wide-refactoring`
완료 보고로 옮기는 안. 파이프라인에는 가장 충실하지만 그 스킬은 선택 호출이라,
리팩토링을 돌리지 않는 흔한 경우에 mutation 제안이 영영 사라진다 — 위 "버림 ②"와
같은 이유로 다시 버렸다. 발견성(한 지점)과 순서(블록 내부) 둘 다 지키는 쪽을 택했다.

**부수 확인 — CRAP 임계값**: 같은 노트 [31:15-35:45]에서 Uncle Bob은 CRAP 임계값을
사람 기준 4에서 에이전트 기준 6(8까지 실험 중)으로 완화한다고 밝혔다. 전역
`crap4java-analyzer`는 8.0 초과를 랭킹하므로 그의 실험 상단과 같아 조정하지 않았다.

**남은 관찰(미조치)**: 같은 노트 [31:15-35:45]에서 Uncle Bob은 "에이전트에게 인간의
**규율**(테스트 한 줄·프로덕션 한 줄의 엄격한 TDD 리듬)을 강제하는 것은 실수지만,
인간의 **가치**를 강제하는 것은 실수가 아니다"라고 주장한다. 이 플러그인의 RGB 강제와
정면으로 부딪히는 관점이며, 플러그인 설계 철학 자체의 문제이므로 이 정정 범위에서는
다루지 않았다.

**교훈 2**: 외부 아이디어를 편입할 때는 "무엇을 가져왔나"뿐 아니라 **"원천에서 어떤
순서로 놓였나"**까지 대조한다. 이 게이트는 도구 3종(CRAP·DRY·mutation)을 정확히
가져왔지만 원천의 단계 경계(Cleaner | Hardener)를 지우고 비용이라는 다른 축으로
다시 묶었고, 그 결과 사용자가 순서대로 실행하면 틀린 순서가 되는 상태가 6일간
남아 있었다.
